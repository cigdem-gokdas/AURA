...
const finalPath = path.join(stagingDir, 'AURA_browser_intermediate.pptx');
...
// 3 — real dashboard screenshot montage
{
  ...
  await shot(s,'position',748,482,469,96);
  await shot(s,'ask',748,587,469,69,{left:0,top:0,right:0,bottom:0.79});
  tx(s,'Gerçek dashboard görüntüsü · çekim anındaki durum · işlem performansı kanıtı değildir',51,683,1080,20,12,C.muted);
  tx(s,'3 / 5',1138,683,78,20,12,C.muted,true,'right');
  notes(s,'Bu slayt AURA dashboard’ının yeni alınmış gerçek ekran görüntülerinden oluşturulmuştur. Sol üstte BTC ve ETH fırsat panosunu; sağda Market Critic ile risk sertifikasını görüyoruz. Sol altta OKX ATK MCP çağrılarının türü, sembolü ve sonucu görünür. Sağ altta çekim anında düz pozisyon durumu ve Ask AURA alanı yer alır. Ekran bir işlem performansı kanıtı değildir.');
}
...
const candidatePath=path.join(stagingDir,'candidate-browser.pptx');
...
console.log(JSON.stringify({font,slideCount:p.slides.items.length,output:'fresh slide PNGs'}));
